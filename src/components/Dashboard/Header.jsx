...
import { Gamepad2, PlusCircle, Layers, LogOut, Search, Trash2, CheckSquare, X } from 'lucide-react';
        ...
        <header className="bg-gray-800/80 backdrop-blur-sm sticky top-0 z-40 shadow-lg">
            ...
            <div className="md:hidden fixed bottom-6 right-6 flex flex-col gap-3 z-50">
                {isSelectionMode ? (
                    <>
                        {selectedGamesCount > 0 && (
                            <button onClick={() => setConfirmBulkDeleteOpen(true)} className="flex items-center justify-center w-14 h-14 bg-red-600 text-white rounded-full shadow-lg hover:bg-red-500 transition-all duration-300">
                                <Trash2 size={28} />
                            </button>
                        )}
                        <button onClick={onToggleSelectionMode} className="flex items-center justify-center w-14 h-14 bg-gray-600 text-white rounded-full shadow-lg hover:bg-gray-500 transition-all duration-300">
                            <X size={28} />
                        </button>
                    </>
                ) : (
                    <>
                        <button onClick={onToggleSelectionMode} className="flex items-center justify-center w-14 h-14 bg-blue-600 text-white rounded-full shadow-lg hover:bg-blue-500 transition-all duration-300">
                            <CheckSquare size={28} />
                        </button>
